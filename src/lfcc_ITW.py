...
MODEL_PATH = "/home/ksathwik/projects/spectral/src/results/models/lfcc_deepsvdd_trump.pt"

OUT_DIR = "results/lfcc_itw_test_only"
os.makedirs(OUT_DIR, exist_ok=True)

# ======================================================
# LFCC PARAMETERS (MUST MATCH TRAINING)
# ======================================================
SR = 16000
DURATION = 10.0
N_FFT = 512
...
# ======================================================
# STEP 2: EXTRACT LFCC EMBEDDINGS
# ======================================================
print("\n[STEP 2] Extracting LFCC (ITW)")
...
for _, row in tqdm(df_trump.iterrows(), total=len(df_trump)):
    ...
    try:
        X.append(extract_lfcc(str(wav_path)))
...
print(f"[LFCC] Bona-fide  : {(y==0).sum()}")
print(f"[LFCC] Spoof      : {(y==1).sum()}")

# ======================================================
# STEP 3: LOAD LFCC DeepSVDD MODEL
# ======================================================
print("\n[STEP 3] Loading LFCC DeepSVDD")

ckpt = torch.load(MODEL_PATH, map_location=DEVICE)

scaler: StandardScaler = ckpt["scaler"]

model = DeepSVDD(N_LFCC).to(DEVICE)
...
plt.legend()
plt.title("LFCC + DeepSVDD — ITW (Donald Trump)")
plt.savefig(os.path.join(OUT_DIR, "score_distribution.png"))
plt.close()

# ======================================================
# STEP 7A: UMAP – RAW LFCC (PLOTLY)
# ======================================================
print("[UMAP] RAW LFCC")
...
px.scatter(
    ...
    y="y",
    color="label",
    title="UMAP – RAW LFCC (ds_wild, Donald Trump)",
    opacity=0.7
).write_html(os.path.join(OUT_DIR, "umap_raw_lfcc.html"))

# ======================================================
# STEP 7B: UMAP – LATENT DeepSVDD (PLOTLY)
# ======================================================
print("[UMAP] LATENT DeepSVDD")
...
Z_lat = reducer.fit_transform(Z_latent)
...
px.scatter(
    ...
    y="y",
    color="label",
    title="UMAP – DeepSVDD Latent Space (ds_wild, Donald Trump)",
    opacity=0.7
).write_html(os.path.join(OUT_DIR, "umap_latent_svdd.html"))

print(f"\n[DONE] All ITW results saved to: {OUT_DIR}")
